...
input.on('noteon', function (msg) {
    ...
    if (msg.note >= 64 && msg.note <= 71) {
        client.send('{"requestType":"playbacks_userInput","cmdline":"","execIndex":' + buttons[msg.note - 64] + ',"pageIndex":' + pageIndex2 + ',"buttonId":0,"pressed":true,"released":false,"type":0,"session":' + session + ',"maxRequests":0}');
    }
            ...
            if (obj.responseSubType == 2) {//Fader LED
                if (wing == 1) {
                    ...
                    for (var i = 0; i < 5; i++) {
                        ...
                        if (ledmatrix[j + 8] != m) {
                            ledmatrix[j + 8] = m;
                            output.send('noteon', { note: j + 8, velocity: m, channel: 0 });
                        }
